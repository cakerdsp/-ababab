...
//自定义的时钟中断函数，实现走马灯显示学号和姓名的效果
extern "C" void my_c_time_interrupt_handler() {
    for(int i = 0; i < 80; i++) {
        stdio.print(0, i, ' ' ,0x07);
    }
    char str[] = "22320021 chenank";
    stdio.moveCursor(0);
    int i = 0;
    int color[7] = {4,6,14,2,3,1,5};
    while(str[i] != '\0') {
        //color = (color * 317 + ) & 0x0f;
        stdio.print(str[i],color[i % 7]);
        //延迟用的
        int j = 1000000;
        while(j) {
            j--;
