...
void ProgramManager::schedule()
{
    ...
    else if (running->status == ProgramStatus::DEAD)
    {
        ...
        // else {
        //     bool flag = true;
        //     ListItem *item_cake = this->allPrograms.head.next;
        //     while (item_cake)
        //     {
        //         PCB *parent = ListItem2PCB(item_cake, tagInAllList);
        //         if (parent->pid == this->running->parentPid)
        //         {
...
void ProgramManager::exit(int ret)
{
    ...
    while (item)
    {
        ...
        if (child->parentPid == this->running->pid)
        {
            ...
            printf("process %d changed its parent from %d to %d\n",child -> pid, this -> running -> pid, this -> running -> parentPid);
        }
        item = item->next;
    }
