...
> 建议使用Intel x86或x86-64架构处理器的机器进行本实验。在使用ARM64架构的处理器（如Apple M1、M2和M3等）时，有可能会在交叉编译等方面遇到一些有趣的问题，也欢迎感兴趣的同学进行挑战。
...
> 若使用的ubuntu版本为22.04，直接使用`apt`安装`nasm`即为`2.15`版本。
...
## gdb调试
> 除了原始的gdb，我们也可以使用gef来增强gdb，从而获得更佳的debug体验。安装教程如下[https://github.com/hugsy/gef?tab=readme-ov-file#instant-setup]。调试的指令与使用gdb时一致。
...
exec /bin/sh
```
> 如果出现`can't open /dev/tty3: No such file or directory`这个报错，一个可能的原因是boot的时候我们提供的init程序执行错误，系统默认执行了`/sbin/init`这一init程序。为此，我们需要在上述init程序的最开头加上 `#!/bin/sh`，指定解析器的位置。
