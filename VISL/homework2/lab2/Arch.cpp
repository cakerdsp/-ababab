...
// 这个没有改Instance的位置，只有addInst改了Instance的位置
bool Block::deleteInst(Instance *i_inst) {
    int n = this->contain.size();
    for(int i = 0; i < n; i++) {
        if (contain[i] == i_inst) {
...
bool FPGA::addInst(int i_x, int i_y, Instance* i_inst){
    ...
    i_inst->setPosition(i_x, i_y);
    return true;
}

bool FPGA::deleteInst(int i_x, int i_y, Instance* i_inst) {
    Block* block = this->getBlock(i_x, i_y);
    if (!block->deleteInst(i_inst)) {
        std::printf("[WARNING] block (%d, %d) have not the instance, the instance is in (%d, %d)\n", i_x, i_y, i_inst->getPosition().first, i_inst->getPosition().second);
        return false;
    }
    return true;
}
