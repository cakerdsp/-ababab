...
class Chess(pygame.sprite.Sprite):
    ...
    def show(self):
        ...
        self.screen.blit(self.image, self.rect)#绘制棋子图片
    ...
    def get_clicked_chess(player, chessboard):
        ...
        for chess in chessboard.get_chess():
            if pygame.mouse.get_pressed()[0] and chess.rect.collidepoint(pygame.mouse.get_pos()):#chess.rect.collidepoint(pygame.mouse.get_pos()):：这行代码首先检查鼠标左键是否被按下 (pygame.mouse.get_pressed()[0])，并且检查当前鼠标位置是否在棋子的矩形区域内 (chess.rect.collidepoint(pygame.mouse.get_pos()))。这个条件检查了鼠标左键是否被点击且点击位置是否在棋子的矩形区域内。
    ...
    def update_position(self, new_row, new_col):
        ...
        self.row = new_row
        self.col = new_col
        self.rect.topleft = (50 + new_col * 57, 50 + new_row * 57)#矩阵左上角的坐标