...
class AccountManager {
  ...
  async login(userId, password, rememberMe = false) {
    try {
      // ===== 特殊调试账户处理 =====
      // 当用户名和密码都是 "admin" 时，触发离线调试模式
      if (userId === 'admin' && password === 'admin') {
        console.log('[AccountManager] Admin login detected. Triggering offline debug mode.');

        const userDataPath = app.getPath('userData');
        const debugFlagPath = path.join(userDataPath, 'debug_mode.flag');

        // 创建标志文件
        fs.writeFileSync(debugFlagPath, '1');
        console.log('[AccountManager] Debug mode flag created at:', debugFlagPath);

        // 返回一个特殊标志，通知主进程重启
        return {
          success: true,
          message: 'Entering offline debug mode. The application will now restart.',
          restart: true
        };
      }
      // ===== 正常登录流程 =====
  ...
  async logout() {
    try {
      ...
      try {
        ...
        console.log('SSE exit notification sent');
      } catch (error) {
        console.error('Failed to send SSE exit notification:', error);
      }
      ...
      this.isInitialized = false;
            ...
            // ===== 彻底清理所有本地存储文件 =====
      console.log('Starting to completely clear local storage files...');

      // 1. 使用UnifiedStorage彻底清理所有数据文件和目录
      const clearResult = this.dataStorage.clearAllDataCompletely();
      if (clearResult.success) {
        console.log(`✅ Local storage directory cleaned up successfully: deleted ${clearResult.deletedFiles} files/directories`);
        if (clearResult.failedFiles > 0) {
          console.warn(`⚠️ ${clearResult.failedFiles} files failed to delete`);
        }
      } else {
        console.error('❌ Failed to clean up local storage directory:', clearResult.message);
      }
      ...
      if (typeof localStorage !== 'undefined') {
        ...
        console.log('Browser local storage cleared');
      }

            // 4. 获取存储统计信息来验证清理结果
      const stats = this.dataStorage.getStorageStats();
      if (stats.totalFiles === 0) {
        console.log('✅ All local files have been successfully deleted');
      } else {
        console.warn(`⚠️  ${stats.totalFiles} files were not deleted, total size: ${stats.totalSize} bytes`);
        console.warn('Remaining file statistics:', stats.categories);
      }

      console.log('User logout complete, all data cleared');
  ...
  async initializeDeviceManagement(userId) {
    try {
      ...
      if (userId === 'admin') {
        ...
        // 4. 初始化数据分析器（离线模式）
        this.sensorDataAnalyzer.setServices(this.apiRegistry, this.deviceStorageManager);
        console.log('🟡 [Offline Debug] Data analyzer initialized (offline mode)');
      ...
      // 5. 初始化SSE管理器
      const sseInitResult = await this.deviceSSEManager.initialize(userId, this.deviceStorageManager);
