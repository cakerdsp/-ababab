...
class StateManager {
    ...
    if (this.listeners.has(stateKey)) {
      this.listeners.get(stateKey).forEach(callback => {
        try {
          ...
          console.error(`状态监听器执行失败 (${stateKey}):`, error);
        }
      });
    }
  }
  ...
  getStateStats() {
    ...
    return {
      ...
      deviceCount: this.getDevicesState().length,
  ...
  importStateData(data) {
    try {
      if (data.storageData) {
        this.storage.importData(data.storageData);
      }
      ...
      if (data.currentState) {
        ...
        Object.keys(this.currentState).forEach(key => {
          this.notifyStateChange(key, this.currentState[key]);
        });
      }

      console.log('状态数据导入成功');
      return true;
    } catch (error) {
      console.error('状态数据导入失败:', error);
      return false;
    }
  }
}
