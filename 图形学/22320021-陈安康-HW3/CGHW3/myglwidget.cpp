...
void MyGLWidget::initializeGL()
{
	...
	// 试过不行，因为向量和法向量共同决定一个点，两个一起才是主码
	//std::vector<vec3> normalData = std::vector<vec3>(objModel.verticesCount + 1);
	//for (int i = 0; i < objModel.triangleCount; ++i) {
	//	for (int j = 0; j < 3; ++j) {
	//		int vertexIndex = objModel.triangles[i][j];
	//		int normalIndex = objModel.triangle_normals[i][j];
	//		normalData[vertexIndex] = objModel.normals_data[normalIndex];
	//		/*auto element = objModel.normals_data[normalIndex];
	//		objModel.normals_data[normalIndex] = objModel.normals_data[vertexIndex];
	//		objModel.normals_data[vertexIndex] = element;*/
	//	}
	//}
	//for (int i = 0; i < objModel.verticesCount; ++i) {
	//		vertexData.push_back(objModel.vertices_data[i].x);
	//		vertexData.push_back(objModel.vertices_data[i].y);
	//		vertexData.push_back(objModel.vertices_data[i].z);
	//		vertexData.push_back(normalData[i].x);
	//		vertexData.push_back(normalData[i].y);
	//		vertexData.push_back(normalData[i].z);
	//}
	//for (int i = 0; i < objModel.triangleCount; ++i) {
	//	for (int j = 0; j < 3; ++j) {
	//		indexs.push_back(objModel.triangles[i][j] - 1);
	//	}
	//}
...
void MyGLWidget::keyPressEvent(QKeyEvent* e) {
	...
	case Qt::Key_Q: degree += 10; update(); break;
	}
}
...
void MyGLWidget::scene_1()
{
	...
	// 自主设置变换矩阵
	camPosition = vec3(300 + objModel.centralPoint.x, 100 + objModel.centralPoint.y, 300 + objModel.centralPoint.z);
	camLookAt = objModel.centralPoint;     // 例如，看向物体中心
	camUp = vec3(0, 1, 0);         // 上方向向量，模型是在xz平面上咯
	// 设置投影
	projMatrix = glm::perspective(radians(20.0f), 1.0f, 0.1f, 2000.0f);
