...
class ImagenetDataset(torchvision.datasets.ImageFolder):
    ...
    def make_dataset(
        ...
        if self.num_labels != -1:
            # Create data_dir if it doesn't exist and save sampled_label_idx.json there
            data_dir = getattr(self, 'data_dir', './data')
            sampled_idx_path = os.path.join(data_dir, 'sampled_label_idx.json')
            os.makedirs(os.path.dirname(sampled_idx_path), exist_ok=True)
            with open(sampled_idx_path, 'w') as f:
                json.dump(lb_idx, f)
        del lb_idx
        gc.collect()
        return instances
...
class SSL_Dataset:
    ...
    def get_ssl_dset(self, num_labels, index=None, include_lb_to_ulb=True,
        ...
        output_file = os.path.join(self.data_dir, "data_statistics")
        output_path = os.path.join(output_file, f"{self.name}_{num_labels}.json")
